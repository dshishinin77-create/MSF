...
# Строгие и точные регулярки для формы SUPPLY
...
def main_func(table_name):
    ...
    # ИСПРАВЛЕНИЕ: 100% надежное определение типа SUPPLY (через in)
    if 'supply' in text_dump or 'поставк' in text_dump:
        doc_mode = 'SUPPLY'
        print(
    ...
    if doc_mode == 'CRD':
            ...
            if section in ['TDD', 'Other_TDD', 'SSC', 'Proposed_docs']:
                ...
                if len(vals) >= 1 and (
                    ...
                    if len(vals) >= 3:
                        ...
                        if isinstance(val, datetime.datetime) or (
                                re.search(r'\d{1,4}[/.-]\d{1,2}[/.-]\d{1,4}',
                                          val_str) and not re.search(
                            r'[a-zа-я]', val_str)):
                            CR_d['General_information']['Reg_date'] = val
                        else:
                            CR_d['General_information']['CR_number'] = val
                    continue
        ...
        for row in range(first_cell[0], last_cell[0] + 1):
            ...
            if section in ['TDD', 'SSC']:
                ...
                if section in ['General', 'Evaluation'] and len(
                        vals) >= 1 and (
                        re_var(['registr'], vals[0]) or re_var(['bejegyz'],
                                                               vals[
                                                                   0]) or re_var(
    ...
    # === ПАРСЕР ДЛЯ SUPPLY (наконец-то запустится) ===
